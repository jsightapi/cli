...
jsight -s convert openapi json test01.jst > /tmp/openapi01.json
if cmp --silent /tmp/openapi01.json ./expected/openapi01.json; then
    echo "> Test 06: [OK]"
else
    echo "> Test 06: [FAILED] \`convert openapi json test01.jst\` output does not match the expected value!\n\n- ACTUAL:\n`head -n 5 /tmp/openapi01.json`\n\n- EXPECTED:\n`head -n 5 ./expected/openapi01.json`"
fi
...
# send statistics
jsight convert openapi json test01.jst 1> /tmp/openapi01.json 2> /dev/null
if cmp /tmp/openapi01.json ./expected/openapi01.json; then
    echo "> Test 08: [OK]"
else
